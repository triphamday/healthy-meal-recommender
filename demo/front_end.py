            ...
            for column, row in zip(st.columns(2), range(2)):
                with column:
                    for idx in range(rows * row, min(rows * (row + 1), len(recommendations))):
                        ...
                        # Lưu ý: backend trả về có 'RecipeID' hay 'RecipeId'? sửa cho thống nhất
                ...
                "series": [
                    {
                        ...
                        "data": [
                            ...
                            for nutrition_value in self.nutrition_values if nutrition_value != 'Calories'
                        ],
                        "emphasis": {
                            "itemStyle": {
                                "shadowBlur": 10,
...
def show_recipe_detail():
    ...
    if recipe_id and 'recommendations' in st.session_state:
        ...
        full_directions = re.sub(r':', '', full_directions)
        full_directions = re.sub(r',\s*,', ',', full_directions)
